...
const blankSeatDraft = (): SeatDraft => ({ zone: "", section_label: "", row_label: "", seat_label: "", external_seat_ref: "", face_value: "", x: "", y: "", allocation_status: "allocated", source_status: "unknown" });
const parseSeatDrafts = (text: string): SeatDraft[] => csvRows(text).map((row) => ({ zone: row.zone || "", section_label: row.section_label || row.section || "", row_label: row.row_label || row.row || "", seat_label: row.seat_label || row.seat || "", external_seat_ref: row.external_seat_ref || "", face_value: row.face_value || "", x: row.x || "", y: row.y || "", allocation_status: row.allocation_status === "not_allocated" ? "not_allocated" : "allocated", source_status: ["available", "sold", "generated", "blocked", "unknown"].includes(row.source_status) ? row.source_status as SeatDraft["source_status"] : "unknown" }));
const seatDraftCsv = (rows: SeatDraft[]) => ["zone,section_label,row_label,seat_label,external_seat_ref,face_value,x,y,allocation_status,source_status", ...rows.filter((row) => row.zone && row.row_label && row.seat_label).map((row) => [row.zone, row.section_label, row.row_label, row.seat_label, row.external_seat_ref, row.face_value, row.x, row.y, row.allocation_status, row.source_status].map((value) => String(value).replaceAll(",", " ")).join(","))].join("\n");
const normalizeSeatDrafts = (rows: SeatDraft[]) => rows.filter((row) => row.zone.trim() && row.row_label.trim() && row.seat_label.trim()).map((row) => ({ zone: row.zone.trim(), section_label: row.section_label.trim() || null, row_label: row.row_label.trim(), seat_label: row.seat_label.trim(), external_seat_ref: row.external_seat_ref.trim() || null, face_value: Number(row.face_value) || null, x: Number.isFinite(Number(row.x)) ? Number(row.x) : null, y: Number.isFinite(Number(row.y)) ? Number(row.y) : null, allocation_status: row.allocation_status, source_status: row.source_status }));
...
export function DirectTicketingScreen({ eventId, apiFetch, canManage, language }: Props) {
  ...
  useEffect(() => { setSeatMapFile(null); setSeatMapSourceNames([]); setSeatMapReview(null); setRescanPending(false); setSeatMapImageUrl(selectedPerformance?.seat_plan_image_url || ""); setShowSeatMapImage(true); setSeatMapZone(""); }, [performanceId, selectedPerformance?.seat_plan_image_url]);
  useEffect(() => {
  ...
  const zoneSummary = useMemo(() => {
    ...
    const source = normalizeSeatDrafts(seatDrafts).map((draft) => { const current = persisted.get(seatDraftKey({ ...draft, section_label: draft.section_label || "" })); return { zone: draft.zone, section_label: draft.section_label || "", face_value: draft.face_value, status: current?.status || "available", allocation_status: draft.allocation_status }; });
    const grouped = new Map<string, { zone: string; section_label: string; total: number; allocated: number; available: number; held: number; issued: number; prices: Set<number> }>();
    source.forEach((seat) => { const key = `${seat.zone}\u0000${seat.section_label}`; const item = grouped.get(key) || { zone: seat.zone || "Unassigned", section_label: seat.section_label, total: 0, allocated: 0, available: 0, held: 0, issued: 0, prices: new Set<number>() }; item.total += 1; if (seat.allocation_status !== "not_allocated") item.allocated += 1; if (seat.status === "available" && seat.allocation_status !== "not_allocated") item.available += 1; if (seat.status === "held") item.held += 1; if (["issued", "checked_in"].includes(seat.status)) item.issued += 1; if (seat.face_value != null && seat.face_value > 0) item.prices.add(seat.face_value); grouped.set(key, item); });
    return Array.from(grouped.values()).sort((left, right) => naturalLabelCompare(`${left.zone} ${left.section_label}`, `${right.zone} ${right.section_label}`)).map((item) => ({ ...item, priceLabel: Array.from(item.prices).sort((a, b) => a - b).map((value) => formatNumber(value)).join(" / ") }));
  }, [seats, seatDrafts]);
  ...
  const resetPerformance = async () => {
    ...
    if (data) { setSeatDrafts([blankSeatDraft()]); setSelectedSeatIds([]); setSeatMapReview(null); setRescanPending(false); setTicketForm((form) => ({ ...form, seat_id: "" })); setMessage(`${t("performanceReset", "Performance reset")} — ${t("removed", "removed")} ${data.seats || 0} ${t("seats", "seats")} ${t("and", "and")} ${data.tickets || 0} ${t("tickets", "tickets")}`); }
  };
  ...
  const applyBatchPrice = () => {
    ...
    const next = seatDrafts.map((row) => (!batchPriceSection || `${row.zone} · ${row.section_label}` === batchPriceSection || row.zone === batchPriceSection ? { ...row, face_value: String(price) } : row));
    const changed = next.filter((row, index) => row.face_value !== seatDrafts[index]?.face_value).length;
  ...
  const discardSeatMapDraft = () => {
    ...
    setSeatMapReview(null); setRescanPending(false); setSeatMapFile(null); setSeatMapSourceNames([]); setSeatMapImageUrl(selectedPerformance?.seat_plan_image_url || "");
  ...
  const renderZoneOverviewCard = (item: (typeof zoneSummary)[number]) => {
    ...
    return <button type="button" key={`${item.zone}-${item.section_label}`} title={details} onClick={() => setSeatMapZone(item.zone)} className="direct-ticketing-zone-overview-card h-full w-full rounded-lg border-2 p-2 text-left transition hover:-translate-y-0.5" style={{ borderColor: colors.border, backgroundColor: colors.fill }}><div className="flex items-start justify-between gap-1"><div className="min-w-0"><p className="truncate text-[11px] font-extrabold" style={{ color: colors.text }}>{item.zone}</p>{item.section_label && <p className="truncate text-[9px] font-semibold" style={{ color: colors.text }}>{item.section_label}</p>}</div><span className="shrink-0 rounded-full bg-white/80 px-1 py-0.5 text-[9px] font-bold" style={{ color: colors.text }}>{item.allocated}/{item.total}</span></div><div className="mt-1 flex items-end justify-between gap-1 text-[9px]"><span className="font-bold text-emerald-700">{item.available} {t("available", "available")}</span><span className="font-extrabold" style={{ color: colors.text }}>{item.priceLabel || "—"}</span></div></button>;
  };
  ...
  return <div className={`direct-ticketing-page space-y-3 ${showTicketSettings ? "" : "ticket-settings-collapsed"}`}>
    <div className="direct-ticketing-page-header sticky top-0 z-30 -mx-2 flex flex-wrap items-center justify-between gap-2 border-b px-2 py-1.5 sm:-mx-4 sm:px-4">
      ...
      {canManage && <div className="flex flex-wrap items-center gap-1.5"><button type="button" disabled={busy} onClick={() => void saveAll()} className="rounded-lg bg-violet-700 px-3 py-1.5 text-[11px] font-extrabold text-white shadow-md shadow-violet-700/20 disabled:cursor-not-allowed disabled:opacity-50">{t("saveAll", "Save all changes")}</button><button type="button" aria-expanded={showTicketSettings} onClick={() => setShowTicketSettings((value) => !value)} className="rounded-lg border border-slate-300 px-2.5 py-1.5 text-[11px] font-bold text-slate-700">{showTicketSettings ? t("hideTicketSettings", "Hide ticket settings") : t("showTicketSettings", "Show ticket settings")}</button>{performanceId && <button type="button" disabled={busy} onClick={() => void resetPerformance()} className="rounded-lg border border-rose-300 px-2.5 py-1.5 text-[11px] font-bold text-rose-700 disabled:cursor-not-allowed disabled:opacity-50">{t("resetPerformance", "Reset performance")}</button>}</div>}
    </div>
            ...
            <div className="relative h-full overflow-hidden rounded-[clamp(8px,2vw,22px)] bg-[#fffaf0]" style={design.direct_ticket_artwork_url && design.direct_ticket_artwork_mode === "background" ? { backgroundImage: `linear-gradient(rgb(255 250 240 / ${100 - Number(design.direct_ticket_artwork_opacity) * 100}%), rgb(255 250 240 / ${100 - Number(design.direct_ticket_artwork_opacity) * 100}%)), url("${design.direct_ticket_artwork_url}")`, backgroundPosition: "center", backgroundSize: "cover" } : undefined}>
              ...
              <div className="grid h-[65%] grid-cols-[1fr_28%] gap-[4%] px-[5%] py-[4%]">
                <div className="min-w-0" style={{ color: "#251b16", WebkitTextFillColor: "#251b16" }}>
                  ...
                  <div className="mt-[3%] grid grid-cols-[.9fr_.9fr_1.35fr_.45fr] gap-[3%] border-t border-slate-200 pt-[2%]"><div><p className="text-[clamp(5px,.7vw,9px)] uppercase text-slate-500">{t("type", "Type")}</p><p className="truncate text-[clamp(7px,1vw,12px)] font-bold">{previewClass?.name || "VIP"}</p></div><div><p className="text-[clamp(5px,.7vw,9px)] uppercase text-slate-500">{t("price", "Price")}</p><p className="truncate text-[clamp(7px,1vw,12px)] font-bold">{formatNumber(Number(previewClass?.price_amount || 1000))} THB</p></div><div><p className="text-[clamp(5px,.7vw,9px)] uppercase text-slate-500">{t("zoneRow", "Zone / Row")}</p><p className="truncate text-[clamp(7px,1vw,12px)] font-bold" style={{ color: previewPrimaryColor, WebkitTextFillColor: previewPrimaryColor }}>ZONE 3 · Row J</p></div><div><p className="text-[clamp(5px,.7vw,9px)] uppercase text-slate-500">{t("seat", "Seat")}</p><p className="truncate text-[clamp(9px,1.25vw,15px)] font-extrabold" style={{ color: previewPrimaryColor, WebkitTextFillColor: previewPrimaryColor }}>23</p></div></div>
    ...
    {zoneSummary.length > 0 && <section className="direct-ticketing-zone-overview rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
      <div className="flex items-center justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-violet-600">{t("zoneOverview", "Zone overview")}</p><h3 className="text-sm font-bold text-slate-900">{t("zoneOverviewTitle", "Seat inventory by zone")}</h3></div><span className="text-[10px] font-semibold text-slate-500">{seatDrafts.filter((row) => row.zone && row.row_label && row.seat_label).length} {t("mappedSeats", "mapped seats")} · {t("dragToArrange", "drag to arrange")}</span></div>
      <div className="direct-ticketing-zone-overview-grid mt-2 grid grid-cols-3 gap-1.5">
    ...
    {(seatMapImageUrl || seatMapGrid.rows.length > 0) && <section className="direct-ticketing-seat-map rounded-2xl border border-slate-300 bg-white p-3 shadow-sm">
