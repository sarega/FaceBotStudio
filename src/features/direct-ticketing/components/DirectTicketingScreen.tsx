...
export function DirectTicketingScreen({ eventId, apiFetch, canManage, language }: Props) {
  ...
  const ticketRecipientSummary = useMemo(() => {
    ...
    filteredTickets.forEach((ticket) => {
      const name = (ticket.holder_name || ticket.buyer_name || "—").trim() || "—";
      const current = groups.get(name) || { name, total: 0, issued: 0, held: 0, checked_in: 0, voided: 0 };
      current.total += 1;
