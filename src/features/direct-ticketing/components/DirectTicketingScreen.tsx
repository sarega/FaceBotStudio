...
export function DirectTicketingScreen({ eventId, apiFetch, canManage, language }: Props) {
  ...
  useEffect(() => { setSeatMapFile(null); setSeatMapSourceNames([]); setSeatMapReview(null); setRescanPending(false); setSeatMapImageUrl(selectedPerformance?.seat_plan_image_url || ""); setShowSeatMapImage(false); setSeatMapZone(""); }, [performanceId, selectedPerformance?.seat_plan_image_url]);
  useEffect(() => {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      if (targetPerformanceId && rows.length) {
        const seatsResponse = await apiFetch("/api/direct-ticketing/seats/import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ event_id: eventId, performance_id: targetPerformanceId, seats: rows, replace_missing: rescanPending, replace_layout: !rescanPending }) });
  ...
  const batchPricingPanel = canManage && seatDrafts.some((row) => row.zone && row.row_label && row.seat_label) ? <section className="direct-ticketing-settings-section direct-ticketing-batch-pricing rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-violet-600">{t("batchPricing", "Batch pricing")}</p><h3 className="mt-1 text-lg font-bold text-slate-900">{t("batchPricingTitle", "Set price for a zone or section")}</h3><p className="mt-1 text-sm text-slate-500">{t("batchPricingHint", "Use this when the source chart did not provide a reliable price. Individual rows can still be edited below.")}</p></div></div><div className="mt-3 grid gap-2 sm:grid-cols-[1fr_1fr_160px_auto] sm:items-end"><label className="text-xs font-bold text-slate-600">{t("zone", "Zone")} / {t("section", "Section")}<select value={batchPriceSection} onChange={(event) => { const section = event.target.value; setBatchPriceSection(section); setBatchPrice(batchPriceValueFor(section)); }} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"><option value="">{t("allScannedSeats", "All scanned seats")}</option>{batchPriceOptions.map((key) => { const price = batchPriceValueFor(key); return <option key={key} value={key}>{key}{price ? ` · ${formatNumber(Number(price))} THB` : ""}</option>; })}</select></label><label className="text-xs font-bold text-slate-600">{t("price", "Price")} (THB)<input type="number" min="0" value={batchPrice} onChange={(event) => setBatchPrice(event.target.value)} placeholder="1500" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></label><button type="button" onClick={applyBatchPrice} className="rounded-lg bg-violet-700 px-3 py-2 text-sm font-bold text-white">{t("applyPrice", "Apply price")}</button><span className="text-xs text-slate-500">{t("batchPriceSaveNote", "Not saved until Save all changes")}</span></div></section> : null;
  ...
  const renderZoneOverviewCard = (item: (typeof zoneSummary)[number]) => {
    ...
    return <button type="button" key={`${item.zone}-${item.section_label}`} title={details} onClick={() => setSeatMapZone(item.zone)} className="direct-ticketing-zone-overview-card h-full w-full rounded-lg border-2 p-2 text-left transition hover:-translate-y-0.5" style={{ borderColor: colors.border, backgroundColor: colors.fill }}><div className="flex items-start justify-between gap-1"><div className="min-w-0"><p className="direct-ticketing-zone-card-accent break-words text-[11px] font-extrabold leading-tight" style={{ color: colors.text }}>{item.zone}</p>{item.section_label && <p className="direct-ticketing-zone-card-accent break-words text-[9px] font-semibold leading-tight" style={{ color: colors.text }}>{item.section_label}</p>}</div><span className="direct-ticketing-zone-card-ratio shrink-0 rounded-full bg-white/80 px-1 py-0.5 text-[9px] font-bold" style={{ color: colors.text }}>{item.allocated}/{item.total}</span></div><div className="mt-1 flex items-end justify-between gap-1 text-[9px]"><span className="font-bold text-emerald-700">{item.available} {t("available", "available")}</span><span className="direct-ticketing-zone-card-accent font-extrabold" style={{ color: colors.text }}>{item.priceLabel || "—"}</span></div></button>;
  };
  ...
  return <div className="direct-ticketing-page space-y-3" data-direct-section={activeSection}>
