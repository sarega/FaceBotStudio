...
function badgeClass(status: string) {
  ...
  if (["contacted", "waiting_reply", "follow_up"].includes(status)) return "bg-amber-50 text-amber-700";
  if (["approved"].includes(status)) return "bg-blue-50 text-blue-700";
  if (["drafted"].includes(status)) return "bg-violet-50 text-violet-700";
  if (["declined", "no_response"].includes(status)) return "bg-rose-50 text-rose-700";
  return "bg-slate-100 text-slate-600";
}

function targetCardClass(status: TargetStatus, selected: boolean) {
  const tone = status === "drafted" ? "border-violet-200 bg-violet-50 hover:bg-violet-100"
    : ["approved"].includes(status) ? "border-blue-200 bg-blue-50 hover:bg-blue-100"
    : ["contacted", "waiting_reply", "follow_up"].includes(status) ? "border-amber-200 bg-amber-50 hover:bg-amber-100"
    : ["replied", "press_kit_sent", "published"].includes(status) ? "border-emerald-200 bg-emerald-50 hover:bg-emerald-100"
    : ["declined", "no_response"].includes(status) ? "border-rose-200 bg-rose-50 hover:bg-rose-100"
    : "border-slate-200 bg-white hover:bg-slate-50";
  return `${tone} ${selected ? "ring-2 ring-blue-400" : ""}`;
}
...
export function OutreachScreen({ eventId, apiFetch, canManage, language }: Props) {
  ...
  const [targetLinks, setTargetLinks] = useState({ facebook_page_url: "", website: "" });
  ...
  const moveTargetSelection = (direction: -1 | 1) => {
    if (!filteredTargets.length) return;
    const currentIndex = Math.max(0, filteredTargets.findIndex((target) => target.id === selectedTargetId));
    const next = filteredTargets[Math.min(filteredTargets.length - 1, Math.max(0, currentIndex + direction))];
    if (!next || next.id === selectedTargetId) return;
    setSelectedTargetId(next.id);
    window.requestAnimationFrame(() => document.getElementById(`outreach-target-${next.id}`)?.scrollIntoView({ block: "nearest" }));
  };
  ...
  useEffect(() => { setTargetLinks({ facebook_page_url: selectedTarget?.facebook_page_url || "", website: selectedTarget?.website || "" }); }, [selectedTarget?.facebook_page_url, selectedTarget?.website, selectedTargetId]);
  useEffect(() => {
    const closeMenu = (event: KeyboardEvent) => { if (event.key === "Escape") setTargetMenuId(""); };
  ...
  const saveTargetLinks = async () => {
    if (!canManage || !selectedTarget) return;
    const data = await request(`/api/outreach/targets/${encodeURIComponent(selectedTarget.id)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...targetPayload(selectedTarget), facebook_page_url: targetLinks.facebook_page_url.trim(), website: targetLinks.website.trim() }) }, t("targetUpdated", "Target updated"));
    if (data?.id) setTargets((current) => current.map((target) => target.id === data.id ? data as Target : target));
  };
  ...
  const deleteTarget = async (target: Target) => {
    if (!canManage || !window.confirm(`${t("deleteTargetConfirm", "Delete target")}: ${target.name}?`)) return;
    const data = await request(`/api/outreach/targets/${encodeURIComponent(target.id)}`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ event_id: eventId }) }, t("targetDeleted", "Target deleted"));
    if (data !== null) await loadCampaignDetail(selectedCampaignId, selectedTargetId === target.id ? "" : selectedTargetId);
  };
  ...
  return (
    <div className="space-y-3" onPointerDown={() => setTargetMenuId("")}>
      ...
      <div className="grid min-w-0 gap-3 xl:h-[calc(100dvh-155px)] xl:grid-cols-[220px_minmax(250px,.75fr)_minmax(420px,1.25fr)]">
        ...
        <section tabIndex={0} onKeyDown={(event) => { if (["ArrowUp", "ArrowDown"].includes(event.key) && !(event.target instanceof HTMLInputElement) && !(event.target instanceof HTMLSelectElement) && !(event.target instanceof HTMLTextAreaElement)) { event.preventDefault(); moveTargetSelection(event.key === "ArrowUp" ? -1 : 1); } }} className="flex min-h-0 min-w-0 flex-col rounded-2xl border border-slate-200 bg-white p-3 outline-none focus-visible:ring-2 focus-visible:ring-blue-400">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-sm font-bold text-slate-900">{t("targetList", "Targets")}</h3><p className="text-[11px] text-slate-500">{selectedCampaign?.name || t("chooseCampaign", "Choose a campaign")}</p></div>{canManage && selectedCampaign && <div className="flex gap-1"><button type="button" disabled={busy} onClick={() => void resolveLinks()} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2 py-1.5 text-[11px] font-bold text-slate-700" title={t("findFacebookPages", "Find Facebook Pages first") }><Search className="h-3.5 w-3.5" />{t("findFacebook", "Find Facebook")}</button><button type="button" onClick={() => setShowTargetForm((value) => !value)} className="rounded-lg bg-blue-600 p-1.5 text-white" aria-label={t("addTarget", "Add target")}><Plus className="h-4 w-4" /></button></div>}</div>
          ...
          <div className="mt-3 min-h-0 flex-1 space-y-1.5 overflow-y-auto pr-1">{filteredTargets.map((target) => <div id={`outreach-target-${target.id}`} key={target.id} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); setSelectedTargetId(target.id); setTargetMenuId(target.id); }} className={`relative w-full rounded-xl border px-3 py-2.5 text-left ${targetCardClass(target.status, selectedTargetId === target.id)}`}><button type="button" onClick={() => setSelectedTargetId(target.id)} className="w-full text-left"><div className="flex items-start justify-between gap-2"><span className="truncate text-sm font-bold text-slate-800">{target.name}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${badgeClass(target.status)}`}>{targetStatusLabel(target.status)}</span></div><p className="mt-1 truncate text-[11px] text-slate-500">{target.organization_type} · {target.delivery_mode === "manual_first_contact" ? t("manualFirstContact", "manual first contact") : target.delivery_mode}</p></button>{targetMenuId === target.id && <div onPointerDown={(event) => event.stopPropagation()} className="absolute right-2 top-2 z-20 w-48 rounded-lg border border-slate-200 bg-white p-1 shadow-xl"><button type="button" onClick={() => { setTargetMenuId(""); void generateTargetDraft(target); }} className="w-full rounded px-2 py-1.5 text-left text-xs hover:bg-slate-50">{t("generate", "Generate draft")}</button><button type="button" onClick={() => { setTargetMenuId(""); void approveTargetDraft(target); }} className="w-full rounded px-2 py-1.5 text-left text-xs hover:bg-slate-50">{t("approveDraft", "Approve draft")}</button><button type="button" onClick={() => { setTargetMenuId(""); void copyTargetDraft(target); }} className="w-full rounded px-2 py-1.5 text-left text-xs hover:bg-slate-50">{t("copy", "Copy draft")}</button><button type="button" onClick={() => { openTargetUrl(target); setTargetMenuId(""); }} className="flex w-full items-center gap-1 rounded px-2 py-1.5 text-left text-xs hover:bg-slate-50"><ExternalLink className="h-3 w-3" />{t("openUrl", "Open URL / find page")}</button><button type="button" onClick={() => { setTargetMenuId(""); void deleteTarget(target); }} className="mt-1 w-full rounded border-t border-slate-200 px-2 py-1.5 text-left text-xs font-bold text-rose-700 hover:bg-rose-50">{t("deleteTarget", "Delete target")}</button><select value={target.status} onChange={(event) => { setSelectedTargetId(target.id); setTargetMenuId(""); void request(`/api/outreach/targets/${encodeURIComponent(target.id)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(targetPayload(target, { status: event.target.value as TargetStatus })) }, t("targetUpdated", "Target updated")).then(() => void loadCampaignDetail(selectedCampaignId, target.id)); }} className="mt-1 w-full rounded border border-slate-200 px-2 py-1.5 text-xs">{Object.entries(statusLabels).map(([value]) => <option key={value} value={value}>{targetStatusLabel(value as TargetStatus)}</option>)}</select></div>}</div>)}{selectedCampaign && filteredTargets.length === 0 && <p className="py-5 text-center text-sm text-slate-500">{t("noTargetsMatch", "No targets match.")}</p>}</div>
        </section>
        ...
        <section className="min-h-0 min-w-0 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-3">
          {!selectedTarget ? <div className="flex min-h-[420px] items-center justify-center text-center text-sm text-slate-500">{t("selectTarget", "Select a target to prepare an outreach message.")}</div> : <>
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><input value={targetLinks.facebook_page_url} onChange={(event) => setTargetLinks({ ...targetLinks, facebook_page_url: event.target.value })} placeholder="Facebook Page URL" className="min-w-0 rounded-lg border border-slate-200 px-2.5 py-2 text-xs" /><input value={targetLinks.website} onChange={(event) => setTargetLinks({ ...targetLinks, website: event.target.value })} placeholder={t("website", "Website URL")} className="min-w-0 rounded-lg border border-slate-200 px-2.5 py-2 text-xs" />{canManage && <button type="button" disabled={busy} onClick={() => void saveTargetLinks()} className="rounded-lg bg-slate-800 px-3 py-2 text-xs font-bold text-white">{t("save", "Save URLs")}</button>}</div>
