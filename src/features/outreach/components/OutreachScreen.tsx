  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[.18em] text-blue-600">{t("pageLabel", "Press & media workflow")}</p>
          <h2 className="mt-1 flex items-center gap-2 text-xl font-bold text-slate-900"><Megaphone className="h-5 w-5 text-blue-600" />{t("title", "Outreach")}</h2>
          <p className="mt-1 max-w-2xl text-sm text-slate-500">{t("description", "AI prepares drafts. A person reviews every first contact and every reply before delivery.")}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-7">
        {[{ label: t("targets", "Targets"), value: counts.total }, { label: t("notContacted", "Not contacted"), value: counts.new }, { label: t("waiting", "Waiting"), value: counts.waiting }, { label: t("replied", "Replied"), value: counts.replied }, { label: t("pressKitSent", "Press Kit sent"), value: counts.pressKit }, { label: t("followUpDue", "Follow-up due"), value: counts.due }, { label: t("published", "Published"), value: counts.published }].map((item) => <div key={item.label} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-slate-400">{item.label}</p><p className="mt-1 text-xl font-bold text-slate-900">{item.value}</p></div>)}
      </div>
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[250px_minmax(260px,0.75fr)_minmax(420px,1.25fr)]">
        ...
        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-3">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-sm font-bold text-slate-900">{t("targetList", "Targets")}</h3><p className="text-[11px] text-slate-500">{selectedCampaign?.name || t("chooseCampaign", "Choose a campaign")}</p></div>{canManage && selectedCampaign && <button type="button" onClick={() => setShowTargetForm((value) => !value)} className="rounded-lg bg-blue-600 p-1.5 text-white" aria-label={t("addTarget", "Add target")}><Plus className="h-4 w-4" /></button>}</div>
          ...
          <div className="mt-3 space-y-1.5">{filteredTargets.map((target) => <button type="button" key={target.id} onClick={() => setSelectedTargetId(target.id)} className={`w-full rounded-xl border px-3 py-2.5 text-left ${selectedTargetId === target.id ? "border-blue-300 bg-blue-50/70" : "border-slate-100 hover:border-slate-200 hover:bg-slate-50"}`}><div className="flex items-start justify-between gap-2"><span className="truncate text-sm font-bold text-slate-800">{target.name}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${badgeClass(target.status)}`}>{targetStatusLabel(target.status)}</span></div><p className="mt-1 truncate text-[11px] text-slate-500">{target.organization_type} · {target.delivery_mode === "manual_first_contact" ? t("manualFirstContact", "manual first contact") : target.delivery_mode}</p></button>)}{selectedCampaign && filteredTargets.length === 0 && <p className="py-5 text-center text-sm text-slate-500">{t("noTargetsMatch", "No targets match.")}</p>}</div>
        </section>
        ...
        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4">
          {!selectedTarget ? <div className="flex min-h-[420px] items-center justify-center text-center text-sm text-slate-500">{t("selectTarget", "Select a target to prepare an outreach message.")}</div> : <>
            <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-blue-600">{t("targetDetail", "Target detail")}</p><h3 className="mt-1 text-lg font-bold text-slate-900">{selectedTarget.name}</h3><p className="mt-1 text-xs text-slate-500">{selectedTarget.organization_type} · {selectedTarget.contact_person || t("noContactPerson", "No contact person")}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${badgeClass(selectedTarget.status)}`}>{targetStatusLabel(selectedTarget.status)}</span></div>
            <div className="mt-3 grid gap-2 rounded-xl bg-amber-50 p-3 text-xs text-amber-900"><p className="font-bold">{eligibility?.eligible ? t("apiReplyEligible", "API reply eligible") : selectedTarget.delivery_mode === "manual_first_contact" ? t("manualFirstContact", "Manual first contact") : selectedTarget.delivery_mode}</p><p>{eligibility?.eligible ? t("approvedReplyWindow", `Approved replies can use the existing sender until ${formatDate(eligibility.eligible_until, language)}`).replace("{date}", formatDate(eligibility.eligible_until, language)) : eligibility?.reason || t("manualFirstContactHint", "AI can prepare the copy, but send the first message in Facebook/Messenger yourself.")}</p><div className="flex flex-wrap gap-2">{selectedTarget.facebook_page_url && <button type="button" onClick={() => window.open(selectedTarget.facebook_page_url, "_blank", "noopener,noreferrer")} className="inline-flex items-center gap-1 font-bold text-amber-800"><ExternalLink className="h-3.5 w-3.5" />{t("openTargetPage", "Open target page")}</button>}{canManage && <><button type="button" onClick={bindIdentity} className="rounded-lg border border-amber-300 px-2.5 py-1.5 font-bold text-amber-800">{t("bindIdentity", "Bind identity")}</button>{["new", "drafted", "approved"].includes(selectedTarget.status) && <button type="button" onClick={() => void updateTarget({ status: "contacted", delivery_mode: selectedTarget.bound_sender_id ? "manual_only" : "manual_first_contact" })} className="inline-flex items-center gap-1 rounded-lg bg-amber-700 px-2.5 py-1.5 font-bold text-white">{t("markContacted", "Mark contacted")}</button>}</>}</div></div>
            <div className="mt-4 grid gap-2 sm:grid-cols-2"><div className="rounded-lg border border-slate-100 px-3 py-2"><p className="text-[10px] font-bold uppercase text-slate-400">{t("facebook", "Facebook")}</p><p className="mt-1 truncate text-xs text-slate-700">{selectedTarget.facebook_page_url || t("notProvided", "Not provided")}</p><p className="mt-1 truncate text-[10px] text-slate-400">{selectedTarget.bound_page_id ? `Page ${selectedTarget.bound_page_id} · sender ${selectedTarget.bound_sender_id}` : t("identityNotBound", "Identity not bound")}</p></div><div className="rounded-lg border border-slate-100 px-3 py-2"><p className="text-[10px] font-bold uppercase text-slate-400">{t("contactTimeline", "Contact timeline")}</p><p className="mt-1 text-xs text-slate-700">{t("firstContact", "First contact")}: {formatDate(selectedTarget.last_contacted_at, language)}</p><p className="mt-1 text-xs text-slate-700">{t("lastReply", "Last reply")}: {formatDate(selectedTarget.last_replied_at, language)}</p></div></div>
            {assignees.length > 0 && canManage && <label className="mt-3 block text-xs font-bold text-slate-700">{t("owner", "Owner")}<select value={selectedTarget.assigned_user_id || ""} onChange={(event) => void updateTarget({ assigned_user_id: event.target.value || null })} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs font-normal"><option value="">{t("unassigned", "Unassigned")}</option>{assignees.map((assignee) => <option key={assignee.id} value={assignee.id}>{assignee.display_name || assignee.username}</option>)}</select></label>}
            {canManage && <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void setFollowUp()} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-bold text-slate-700">{selectedTarget.next_follow_up_at ? `${t("followUp", "Follow-up")}: ${formatDate(selectedTarget.next_follow_up_at, language)}` : t("setFollowUp", "Set follow-up")}</button><button type="button" onClick={() => void setOutcome("published")} className="rounded-lg border border-emerald-200 px-2.5 py-1.5 text-xs font-bold text-emerald-700">{t("publishedOutcome", "Published")}</button><button type="button" onClick={() => void setOutcome("declined")} className="rounded-lg border border-rose-200 px-2.5 py-1.5 text-xs font-bold text-rose-700">{t("declined", "Declined")}</button><button type="button" onClick={() => void setOutcome("no_response")} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-bold text-slate-600">{t("noResponse", "No response")}</button></div>}
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-2"><div><h4 className="text-sm font-bold text-slate-900">{t("aiDraft", "AI draft")}</h4><p className="text-[11px] text-slate-500">{t("draftReviewHint", "Every revision stays unapproved until a person reviews it.")}</p></div><div className="flex flex-wrap gap-2">{canManage && <><button type="button" disabled={busy} onClick={() => void saveDraft("generate")} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white"><Sparkles className="h-3.5 w-3.5" />{t("generate", "Generate")}</button>{["replied", "follow_up"].includes(selectedTarget.status) && <button type="button" disabled={busy || !selectedTarget.bound_sender_id} onClick={() => void saveDraft("suggested_reply")} className="inline-flex items-center gap-1.5 rounded-lg bg-violet-600 px-3 py-2 text-xs font-bold text-white"><Sparkles className="h-3.5 w-3.5" />{t("suggestedReply", "Suggested reply")}</button>}<button type="button" disabled={busy || !draftBody.trim()} onClick={() => void saveDraft("manual")} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700"><Save className="h-3.5 w-3.5" />{t("saveRevision", "Save revision")}</button></>}</div></div>
            ...
            <div className="mt-2 flex flex-wrap items-center justify-between gap-2"><div className="flex flex-wrap gap-2">{canManage && <button type="button" disabled={!draftBody.trim()} onClick={() => void copyDraft()} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-bold text-slate-600"><Copy className="h-3.5 w-3.5" />{t("copy", "Copy")}</button>}{canManage && drafts[0]?.approval_status !== "approved" && <button type="button" disabled={!drafts[0] || draftDirty} onClick={() => void approveDraft()} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-2.5 py-1.5 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><Check className="h-3.5 w-3.5" />{t("approveDraft", "Approve draft")}</button>}{canManage && drafts[0]?.kind === "suggested_reply" && drafts[0]?.approval_status === "approved" && <button type="button" disabled={busy || !eligibility?.eligible} onClick={() => void sendApprovedReply()} className="rounded-lg bg-emerald-700 px-2.5 py-1.5 text-xs font-bold text-white disabled:opacity-50">{t("approveSendReply", "Approve & Send reply")}</button>}</div><span className="text-[11px] text-slate-400">{draftDirty ? t("saveEditedBeforeApproval", "Save this edited revision before approval") : drafts[0] ? `Revision ${drafts[0].revision} · ${drafts[0].kind} · ${drafts[0].approval_status}` : t("noRevision", "No revision yet")}</span></div>
